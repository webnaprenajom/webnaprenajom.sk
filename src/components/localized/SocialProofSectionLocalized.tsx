...
const stats_sk = [
  ...
  { target: 0, suffix: "€", label: "vstupné náklady" },
];

const stats_en = [
  { target: 100, suffix: "+", label: "websites created" },
  { target: 48, suffix: "h", label: "average launch" },
  { target: 98, suffix: "%", label: "client satisfaction" },
  { target: 0, suffix: "€", label: "upfront costs" },
];

const stats_de = [
  { target: 100, suffix: "+", label: "erstellte Websites" },
  { target: 48, suffix: "h", label: "durchschnittlicher Start" },
  { target: 98, suffix: "%", label: "Kundenzufriedenheit" },
  { target: 0, suffix: "€", label: "Vorabkosten" },
];

const statsMap = { sk: stats_sk, en: stats_en, de: stats_de };
...
const realizationsLabel = { sk: "Naše realizácie", en: "Our Portfolio", de: "Unsere Projekte" };
...
const AnimatedStat = ({ stat, index, inView }: { stat: typeof stats_sk[0]; index: number; inView: boolean }) => {
...
const SocialProofSectionLocalized = () => {
    ...
    <section className="py-20 section-accent-glow">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="text-center mb-8">
          <p className="text-sm font-medium text-muted-foreground uppercase tracking-widest">{realizationsLabel[lang]}</p>
        </div>
          ...
          <div className="flex justify-center gap-2 mt-6">
            {count > 0 && Array.from({ length: count }).map((_, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-300 ${current === i ? "bg-primary w-6" : "bg-muted-foreground/30 hover:bg-muted-foreground/50 w-2"}`}
...
export default SocialProofSectionLocalized;
