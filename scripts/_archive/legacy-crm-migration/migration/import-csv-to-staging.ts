...
import { buildReconciliation } from "./lib/reconciliation.js";
import { writeMigrationReports } from "./lib/report.js";
import { MIGRATION_SOURCES, resolveSourceFile, stagingTableFor } from "./lib/sources.js";
...
async function upsertStagingRows(
  ...
  const def = MIGRATION_SOURCES.find((s) => s.sourceFile === file.sourceFile);
  const table = def ? stagingTableFor(def) : file.kind === "finance" ? "legacy_finance_staging" : "legacy_import_rows";
...
async function main() {
  ...
  const reconciliation = buildReconciliation(allStaged);

  const wouldPromote = reconciliation.promotePlan
    .filter((p) => p.promotePolicy === "migrate")
    .reduce((n, p) => n + p.rowCount, 0);
  const wouldSkip = reconciliation.promotePlan
    .filter((p) => p.promotePolicy === "skip")
    .reduce((n, p) => n + p.rowCount, 0);
  const manualReview = reconciliation.promotePlan
    .filter((p) => p.promotePolicy === "manual")
    .reduce((n, p) => n + p.rowCount, 0);
  ...
  const report: MigrationReport = {
    ...
    analysis: {
      ...
      customerEmailCollisions: analysis.customerEmailCollisions,
    ...
    totals: {
      ...
      financeRowsStaged,
      reviewItems: analysis.allReviewItems.length + reconciliation.identityGaps.length,
      wouldPromote,
      wouldSkip,
      manualReview,
    },
  ...
  console.log(`  Manual review rows: ${report.totals.manualReview}`);
  console.log(`  Would promote: ${report.totals.wouldPromote}`);
  console.log(`  Would skip: ${report.totals.wouldSkip}`);
