-- Destructive delete: precheck + execute RPCs (customer, hosting, rental_website)
-- Hard block on payment_fact / cost_fact / payout_fact
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(
    private.destructive_blocking_record(
      pr.id, 'payment_fact', 'payment_records',
      'Platba ' || pr.amount::text || ' €',
      pr.amount,
  ...
  WHERE pr.source_table = 'hosting_records'
    AND pr.source_id = p_entity_id::text
    AND pr.truth_level = 'payment_fact';

  SELECT COUNT(*) INTO v_legacy_payments
  FROM payment_records pr
  WHERE pr.source_table = 'hosting_records'
    AND pr.source_id = p_entity_id::text
    AND pr.truth_level = 'legacy_import';
  ...
  RETURN jsonb_build_object(
    ...
    'block_reason', CASE WHEN jsonb_array_length(v_blocking) > 0
      THEN 'Hosting má potvrdené finančné platby (payment_fact). Najprv ich vyriešte vo Finance.'
      ELSE NULL END,
...
DECLARE
  ...
  v_blocking jsonb := '[]'::jsonb;
  v_payment_block jsonb;
  v_cost_block jsonb;
  v_payout_block jsonb;
  v_rp_count int;
  v_comm_count int;
  v_hosting_count int;
  v_legacy_pay int;
...
BEGIN
  ...
  FROM payment_records pr
  WHERE pr.rental_website_id = p_entity_id AND pr.truth_level = 'payment_fact';
  ...
  WHERE cr.rental_website_id = p_entity_id AND cr.truth_level = 'cost_fact';
  ...
  SELECT COALESCE(jsonb_agg(
    private.destructive_blocking_record(
      ...
      po.amount,
      to_char(po.paid_at AT TIME ZONE 'UTC', 'YYYY-MM-DD'),
  ...
  WHERE c.source_type = 'rental' AND c.source_id = p_entity_id::text
    AND po.truth_level = 'payout_fact';
  ...
  SELECT COUNT(*) INTO v_comm_count FROM commissions
  ...
  SELECT COUNT(*) INTO v_legacy_pay FROM payment_records
    WHERE rental_website_id = p_entity_id AND truth_level = 'legacy_import';
  SELECT COUNT(*) INTO v_legacy_cost FROM cost_records
    WHERE rental_website_id = p_entity_id AND truth_level = 'legacy_import';
  ...
  RETURN jsonb_build_object(
    ...
    'can_delete', jsonb_array_length(v_blocking) = 0,
    'block_reason', CASE WHEN jsonb_array_length(v_blocking) > 0
      THEN 'Prenájom má potvrdené finančné fakty. Najprv ich vyriešte vo Finance.'
      ELSE NULL END,
    'finance_critical', jsonb_array_length(v_blocking) > 0,
...
BEGIN
  ...
  -- payment_fact by email or rental
  ...
  RETURN jsonb_build_object(
    ...
    'can_delete', jsonb_array_length(v_blocking) = 0,
    'block_reason', CASE WHEN jsonb_array_length(v_blocking) > 0
      THEN 'Klient má potvrdené finančné fakty. Najprv ich vyriešte vo Finance.'
      ELSE NULL END,
    'finance_critical', jsonb_array_length(v_blocking) > 0,
    'sections', jsonb_build_array(
...
BEGIN
  ...
  WHERE source_table = 'hosting_records'
    AND source_id = p_entity_id::text
    AND truth_level = 'legacy_import';
...
BEGIN
  ...
  UPDATE commissions
  SET source_type = NULL, source_id = NULL, updated_at = now()
  WHERE source_type = 'rental' AND source_id = p_entity_id::text;
  ...
  RETURN jsonb_build_object(
    ...
    'deleted', jsonb_build_object(
      'rental_websites', 1,
      'rental_payments', v_del_rp,
      'legacy_payments', v_del_pay,
      'legacy_costs', v_del_cost
    ),
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'detached', jsonb_build_object(
      'note', 'FK ON DELETE SET NULL applied to linked CRM entities'
    )
  );
END;
$$;
...
COMMENT ON FUNCTION public.admin_precheck_destructive_delete IS
  'Returns impact summary + blocking finance facts before destructive delete.';
COMMENT ON FUNCTION public.admin_execute_destructive_delete IS
  'Transactional destructive delete with finance integrity guards.';
