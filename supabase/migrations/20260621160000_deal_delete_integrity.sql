-- Deal delete integrity: block project/hosting/marketing delete when linked payments or commissions exist.
...
BEGIN
  ...
  FROM commissions c
  WHERE c.source_type = 'project' AND c.source_id = p_entity_id::text;
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_comm_count
  FROM commissions c
  WHERE c.source_type = 'marketing' AND c.source_id = p_entity_id::text;
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_comm_count
  FROM commissions c
  WHERE c.source_type = 'hosting' AND c.source_id = p_entity_id::text;
...
BEGIN
  ...
  WHERE parent_type = 'project' AND parent_id = p_entity_id::text;
...
BEGIN
  ...
  UPDATE tasks
  SET parent_type = NULL, parent_id = NULL
  WHERE parent_type = 'marketing' AND parent_id = p_entity_id::text;
  GET DIAGNOSTICS v_det_tasks = ROW_COUNT;
...
BEGIN
  ...
  UPDATE tasks
  SET parent_type = NULL, parent_id = NULL
  WHERE parent_type = 'hosting' AND parent_id = p_entity_id::text;
  GET DIAGNOSTICS v_det_tasks = ROW_COUNT;
...
BEGIN
  ...
  CASE p_entity_type
    ...
    WHEN 'project' THEN RETURN private.precheck_project_delete(p_entity_id);
    WHEN 'marketing' THEN RETURN private.precheck_marketing_delete(p_entity_id);
...
BEGIN
  ...
  CASE p_entity_type
    ...
    WHEN 'project' THEN RETURN private.execute_project_delete(p_entity_id);
    WHEN 'marketing' THEN RETURN private.execute_marketing_delete(p_entity_id);
...
COMMENT ON FUNCTION private.precheck_project_delete IS
  'Blocks project delete when linked payment_records or commissions exist.';
COMMENT ON FUNCTION private.precheck_marketing_delete IS
  'Blocks marketing delete when linked payment_records or commissions exist.';
